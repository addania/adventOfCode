...
export const CodeAdvent20190602 = () => {
  ...
  return (
    <div>
      ...
      <button style={{ backgroundColor: "#69747C" }}><span onClick={handleClick}>Calculate orbits</span>
      </button>      
    </div>
  );
};
...
function countOrbits(inputOrbitsDictionary){
  ...
  let recursivePathYOU=recursionPath(inputOrbitsDictionary, "YOU", pathArrayYOU);
  let recursivePathSAN=recursionPath(inputOrbitsDictionary, "SAN", pathArraySAN);

  //console.log("recurvise path you", recursivePathYOU);
  //console.log("recurvise path san", recursivePathSAN);
  ...
  //console.log("firstCommonOrbit",firstCommonOrbit);
  let countYOU=0;
  let forwardYOU=true;
  for (let j=0;j<recursivePathYOU.length;j++){
