...
function checkOvelaps(inputGrid, inputWireInfo){
  ...
  newGrid[inputWireInfo.centralPortPositionVertical][inputWireInfo.centralPortPositionHorizontal]="X";
  ...
  for (let wirePosition=0;wirePosition<inputWireInfo.wire2.length;wirePosition++){
    ...
    if (inputWireInfo.wire2[wirePosition].direction === "R"){
      ...
      for (let iterate=1;iterate<=inputWireInfo.wire2[wirePosition].movement;iterate++){
...
function calculateMinDistance(overlapsInput, wireInfoInput){
  ...
  for (let overlapping=0;overlapping<overlapsInput.length;overlapping++){
    let distance=Math.abs(overlapsInput[overlapping][0]- wireInfoInput.centralPortPositionVertical) + Math.abs(overlapsInput[overlapping][1] - wireInfoInput.centralPortPositionHorizontal);
