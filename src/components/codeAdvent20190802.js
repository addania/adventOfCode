...
export const CodeAdvent20190802 = () => {
  ...
  function handleClick() {
   ...
   let computedLayerInfo=computeLayerInfo(imageData,imageSize,numberOfLayers, imageWidth, imageHeight);
...
function computeLayerInfo(imageDataInput,imageSizeInput,numberOfLayersInput, imageWidthInput, imageHeightInput){
  let layersArray=[];
  ...
  for (let position=0;position<layersArray[0].length;position++){
    for (let layer=0;layer<layersArray.length;layer++){
     if(layersArray[layer][position]==1 || layersArray[layer][position]==0){
